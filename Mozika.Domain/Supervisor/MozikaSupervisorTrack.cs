...
namespace Mozika.Domain.Supervisor
{
    public partial class MozikaSupervisor
    {
        ...
        public async Task<IEnumerable<TrackApiModel>> GetAllTrackAsync()
        {
            IEnumerable<TrackApiModel> tracks = await _trackRepository.GetAllAsync().ContinueWith(
                it => it.Result.ConvertAll()
            );
            return tracks;
        }
        ...
        public IEnumerable<TrackApiModel> GetAllTrackByName(string name)
        {
            var tracks = _trackRepository.GetAllByName(name);
            return tracks.ConvertAll();
        }
    }
}